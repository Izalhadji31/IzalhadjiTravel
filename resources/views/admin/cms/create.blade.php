...
<div class="page-header">
    <h1 class="page-title">Create CMS Page</h1>
    <p class="page-subtitle">Add a new static page</p>
</div>
...
<div class="card" style="max-width: 900px;">
    <form method="POST" action="{{ route('cms.store') }}">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-bottom: 1rem;">
            ...
            <div>
                <label class="form-label" for="slug">Slug (auto-generated if empty)</label>
                <input type="text" id="slug" name="slug" value="{{ old('slug') }}" class="form-input" placeholder="my-page-slug">
                @error('slug')
                    <div class="form-error">{{ $message }}</div>
                @enderror
            </div>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 1rem; margin-bottom: 1rem;">
            ...
            <div>
                ...
                <input type="number" id="order" name="order" value="{{ old('order', 0) }}" class="form-input" min="0">
                @error('order')
                    <div class="form-error">{{ $message }}</div>
                @enderror
            </div>
            <div>
                ...
                <input type="url" id="image_url" name="image_url" value="{{ old('image_url') }}" class="form-input" placeholder="https://...">
                @error('image_url')
                    <div class="form-error">{{ $message }}</div>
                @enderror
            </div>
        ...
        <div style="margin-bottom: 1rem;">
            ...
            <textarea id="excerpt" name="excerpt" class="form-input" rows="2" placeholder="Short summary...">{{ old('excerpt') }}</textarea>
            @error('excerpt')
                <div class="form-error">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div style="margin-bottom: 1rem;">
            ...
            <div style="border: 1px solid #e5e7eb; border-radius: 0.5rem; overflow: hidden;">
                ...
                <div id="editor" contenteditable="true" style="min-height: 300px; padding: 1rem; outline: none; font-size: 0.95rem; line-height: 1.6;">{{ old('content') }}</div>
            </div>
