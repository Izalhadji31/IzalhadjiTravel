...
<body>
    ...
    <div class="register-card">
        ...
        @else
            {{-- Register Form --}}
            <form method="POST" action="{{ route('register.store') }}" id="registerForm" data-old-phone="{{ old('phone') }}">
            @csrf
        ...
        <a href="{{ $googleConfigured ? route('auth.google') : '#' }}"
           class="btn-google"
           @if(!$googleConfigured) onclick="event.preventDefault(); alert('{{ addslashes($googleDisabledMessage) }}');" @endif>
    ...
    (function() {
        ...
        if (registerForm) {
            ...
            var oldPhone = document.getElementById('registerForm')?.getAttribute('data-old-phone');
            if (oldPhone) {
